...
def qwen_pooling_attn(
		...
		logger.warning_once(
			"------------------this is in customized qwen2 attention forward--------------"
		)
		...
		attn_output = pooling_attention(query_states, key_states, value_states,
								  ...
								  cos, sin, position_ids, past_key_value, output_attentions, use_cache, cache_position)
...
def pooling_attention_wo_cache(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor,
	...
	if not use_cache:
		num_pool = (num_pool//64) * 64
	if num_pool>0:
		pooling_len = int(num_pool * pooling_size)
		window_size = q_len-pooling_len
...
def replace_qwen_attn_forward(p_size = 16, w_size = 1024, pool_func='cca'):
	global pooling_size
	global window_size
	global pooling_func
	pooling_size = p_size
	...
	for v in transformers.models.qwen2.modeling_qwen2.QWEN2_ATTENTION_CLASSES.values():
		v.forward = qwen_pooling_attn
	print(f'replace qwen attention forward with simple pooling forward {pooling_size=}, {window_size=}, {pooling_func=}')